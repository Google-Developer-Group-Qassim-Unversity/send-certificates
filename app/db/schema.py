...
from sqlalchemy import Column, DateTime, Enum, ForeignKeyConstraint, Index, Integer, JSON, String, text
...
class Actions(SQLModel, table=True):
    ...
    action_name: str = Field(sa_column=Column('action_name', VARCHAR(60), nullable=False))
    points: int = Field(sa_column=Column('points', INTEGER, nullable=False))
    action_type: ActionsActionType = Field(sa_column=Column('action_type', Enum(ActionsActionType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    ar_action_name: str = Field(sa_column=Column('ar_action_name', VARCHAR(100), nullable=False))
...
class Departments(SQLModel, table=True):
    ...
    type: DepartmentsType = Field(sa_column=Column('type', Enum(DepartmentsType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    ar_name: str = Field(sa_column=Column('ar_name', VARCHAR(100), nullable=False))

    departments_logs: list['DepartmentsLogs'] = Relationship(back_populates='department')
...
class Events(SQLModel, table=True):
    ...
    location_type: EventsLocationType = Field(sa_column=Column('location_type', Enum(EventsLocationType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    location: str = Field(sa_column=Column('location', VARCHAR(100), nullable=False))
    start_datetime: datetime.datetime = Field(sa_column=Column('start_datetime', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
    end_datetime: datetime.datetime = Field(sa_column=Column('end_datetime', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
    status: EventsStatus = Field(sa_column=Column('status', Enum(EventsStatus, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    description: Optional[str] = Field(default=None, sa_column=Column('description', TEXT))
    image_url: Optional[str] = Field(default=None, sa_column=Column('image_url', VARCHAR(100)))
    is_official: Optional[int] = Field(default=None, sa_column=Column('is_official', TINYINT(1), server_default=text("'0'")))
...
class Members(SQLModel, table=True):
    ...
    gender: MembersGender = Field(sa_column=Column('gender', Enum(MembersGender, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    uni_level: int = Field(sa_column=Column('uni_level', Integer, nullable=False))
    uni_college: str = Field(sa_column=Column('uni_college', VARCHAR(100), nullable=False))
    created_at: datetime.datetime = Field(sa_column=Column('created_at', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
    updated_at: datetime.datetime = Field(sa_column=Column('updated_at', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
    is_authenticated: int = Field(sa_column=Column('is_authenticated', TINYINT(1), nullable=False, server_default=text("'0'")))
    email: Optional[str] = Field(default=None, sa_column=Column('email', String(100)))
    phone_number: Optional[str] = Field(default=None, sa_column=Column('phone_number', String(20)))
...
class Forms(SQLModel, table=True):
    ...
    form_type: FormsFormType = Field(sa_column=Column('form_type', Enum(FormsFormType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    google_form_id: Optional[str] = Field(default=None, sa_column=Column('google_form_id', VARCHAR(100)))
    google_refresh_token: Optional[str] = Field(default=None, sa_column=Column('google_refresh_token', VARCHAR(500)))
    google_watch_id: Optional[str] = Field(default=None, sa_column=Column('google_watch_id', String(100)))
    google_responders_url: Optional[str] = Field(default=None, sa_column=Column('google_responders_url', VARCHAR(150)))
...
class Role(SQLModel, table=True):
    ...
    role: RoleRole = Field(sa_column=Column('role', Enum(RoleRole, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
...
class DepartmentsLogs(SQLModel, table=True):
    ...
    id: int = Field(sa_column=Column('id', INTEGER, primary_key=True))
    department_id: int = Field(sa_column=Column('department_id', INTEGER, nullable=False))
...
class MembersLogs(SQLModel, table=True):
    ...
    member_id: int = Field(sa_column=Column('member_id', INTEGER, nullable=False))
    log_id: int = Field(sa_column=Column('log_id', INTEGER, nullable=False))
    date: datetime.datetime = Field(sa_column=Column('date', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
...
class Modifications(SQLModel, table=True):
    ...
    id: int = Field(sa_column=Column('id', INTEGER, primary_key=True))
    log_id: int = Field(sa_column=Column('log_id', INTEGER, nullable=False))
    type: ModificationsType = Field(sa_column=Column('type', Enum(ModificationsType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
...
class Submissions(SQLModel, table=True):
    ...
    member_id: int = Field(sa_column=Column('member_id', INTEGER, nullable=False))
    is_accepted: int = Field(sa_column=Column('is_accepted', TINYINT(1), nullable=False, server_default=text("'0'")))
    submitted_at: datetime.datetime = Field(sa_column=Column('submitted_at', DateTime, nullable=False, server_default=text('CURRENT_TIMESTAMP')))
    submission_type: SubmissionsSubmissionType = Field(sa_column=Column('submission_type', Enum(SubmissionsSubmissionType, values_callable=lambda cls: [member.value for member in cls]), nullable=False))
    google_submission_id: Optional[str] = Field(default=None, sa_column=Column('google_submission_id', String(100)))
    google_submission_value: Optional[dict] = Field(default=None, sa_column=Column('google_submission_value', JSON))
